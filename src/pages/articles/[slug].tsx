    ...
    <Layout>
      <div style={{ position: "relative", height: 420, overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", inset: 0, padding: "48px 24px", background: "linear-gradient(to top, rgba(18,25,56,.97) 0%, transparent 60%)", display: "flex", flexDirection: "column", justifyContent: "flex-end" }}>
          ...
          <h1 style={{ fontSize: "clamp(20px, 4vw, 32px)", fontWeight: 800, lineHeight: 1.15, color: "#fff", marginBottom: 12, maxWidth: 800 }}>{article.titre}</h1>
          <div style={{ fontSize: 12, color: "rgba(255,255,255,.5)", display: "flex", gap: 12, flexWrap: "wrap" }}>
            {article.auteur && <strong style={{ color: "rgba(255,255,255,.85)", fontWeight: 500 }}>{article.auteur}</strong>}
...
export const getStaticPaths: GetStaticPaths = async () => {
  ...
  const paths = (db.articles || []).filter((a: any) => a.statut === "Publié" && a.slug).map((a: any) => ({ params: { slug: a.slug } }));
